    ...
    return (
        <div className="AddListingContainer editListingWrapper">
            ...
            <div className="addListingTop">
                <p className="small-caps-purple">Edit Your Listing</p>
            </div>
            <form className="Form addListingForm" onSubmit={submit}>
                <label htmlFor="tagline-input">Tagline</label>
                    <input id="tagline-input" maxLength="35" required className={`text-input ${!listingDetails.tagline ? '' : 'lowercase'}`} type="text" name="tagline" value={listingDetails.tagline} placeholder='Enter catchy title for your listing' onChange={e => setListingDetails({...listingDetails, tagline: e.target.value})} />
                ...
                <label htmlFor="file-upload">Upload one or more photos. <br /><span style={{fontSize: "0.83em", color: "black"}}>This will overwrite any existing photos.</span></label>
