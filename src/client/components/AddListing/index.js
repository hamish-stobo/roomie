    ...
    return (
        <div className="AddListingContainer">
            ...
            <form className="Form addListingForm" onSubmit={submit}>
                <input required maxLength="35" className={`text-input ${!listingDetails?.tagline ? '' : 'lowercase'}`} type="text" name="tagline" placeholder='Enter a catchy title for your listing' value={listingDetails?.tagline} onChange={onChange} />
                <input required className={`text-input ${!listingDetails?.listing_location ? '' : 'lowercase'}`} type="text" name="listing_location" placeholder="Where is the flat located?" value={listingDetails?.listing_location} onChange={onChange} />
                <input required className="text-input" type="number" name="rent" placeholder="What is the weekly rent?" value={!listingDetails?.rent ? undefined : listingDetails?.rent} onChange={onChange} />
                <input required className="text-input" type="number" name="bedrooms" placeholder="How many bedrooms?" value={!listingDetails?.bedrooms ? undefined : listingDetails?.bedrooms} onChange={onChange} />
                <input required className="text-input" type="number" name="bathrooms" placeholder="How many bathrooms?" value={!listingDetails?.bathrooms ? undefined : listingDetails?.bathrooms} onChange={onChange} />
