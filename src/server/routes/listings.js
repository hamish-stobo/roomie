...
const {
    ...
    createListing,
    deleteListing,
} = require('../db/dbfunctions/listings')
const {
    validateToken,
    getUserIdFromToken,
    compareIDs,
...
router.get('/:listing_id', async (req, res) => {
    try {
        ...
        if (!listing || JSON.stringify(listing) == '{}')
            throw "There's no listing by that name around these parts. Now, be on your way."
        res.status(200).send(JSON.stringify(listing))
    } catch (e) {
        e ===
        "There's no listing by that name around these parts. Now, be on your way."
...
router.post('/:user_id', validateToken, async (req, res) => {
    try {
        ...
        if (JSON.stringify(body) === '{}' || data.length <= 0) {
            ...
            const insertListingResponse = await createListing(
                user_id,
                body,
                data
            )
...
router.get('/likes/:user_id', async (req, res) => {
    try {
        ...
        if (!likedListings || !Array.isArray(likedListings))
            throw "Couldn't find that"
