    ...
    try {
        ...
        listing.images = !!images
            ? await Promise.all(
                  images.map(
                      async (image) =>
                          await convertToBase64(image.listing_image)
                  )
              )
            : []
        return listing
...
const getAllListings = async (db = conn) => {
    try {
        const listingsArr = await db('listings').select()
        const imagesObj = {}
...
const deleteListing = async (listing_id, db = conn) => {
    try {
        ...
        const listingImagesDeletion = await db('images')
            .del()
            .where('images_listing_id', listing_id)
        if (listingImagesDeletion == 0)
            throw `${listingImagesDeletion} listingsImages were deleted`
        const listingDeletion = await db('listings')
            .del()
            .where('listing_id', listing_id)
        if (listingDeletion == 0)
            throw `${listingDeletion} listings were deleted`
        console.log(
            `Success! ${listingDeletion} listings were removed, it had ${listingImagesDeletion} photos and ${likesDeletion} likes`
        )
        return 'Success!'
    } catch (e) {
        console.error(e)
...
const getListingsLiked = async (user_id) => {
    try {
        ...
        const filteredListings = allListings.filter((listing) =>
            listingIdsObj.hasOwnProperty(listing.listing_id)
        )
