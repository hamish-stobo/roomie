...
const createUser = async (userToInsert, db = conn) => {
    try {
        ...
        if (!userInsert[0] || JSON.stringify(userInsert[0]) === '{}')
            throw 'Insert of user failed'
        // console.log(userInsert[0])
        userInsert[0].profile_picture = await convertToBase64(
            userInsert[0].profile_picture
        )
        return userInsert[0]
    } catch (e) {
...
const updateUser = async (userID, user, db = conn) => {
    try {
        ...
        updateUser[0].profile_picture = await convertToBase64(
            updateUser[0].profile_picture
        )
        return updateUser[0]
    } catch (e) {
...
const deleteUser = async (user_id, db = conn) => {
    try {
        ...
        if (getListings.length > 0) {
            ...
            images = getListings.forEach(
                async (listing) =>
                    await db('images').del().where('images_listing_id', listing)
            )
            delListings = await db('listings')
                .del()
                .where('listings_user_id', user_id)
        }
        const users = await db('users').del().where('user_id', user_id)
        console.log(
            `${users} users were deleted, along with ${delListings} listings, ${images} listing photos and ${likesDeleted} likes`
        )
        return 'Success!'
    } catch (e) {
